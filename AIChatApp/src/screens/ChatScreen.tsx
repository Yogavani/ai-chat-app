...
const saveChatReadCutoff = async (contactUserId: number, timestampMs = Date.now()) => {
  try {
    const raw = await AsyncStorage.getItem(CHAT_READ_CUTOFFS_KEY);
    const parsed = raw ? JSON.parse(raw) : {};
    const existing = Number(parsed?.[String(contactUserId)] || 0);
    parsed[String(contactUserId)] = Math.max(existing, timestampMs);
    await AsyncStorage.setItem(CHAT_READ_CUTOFFS_KEY, JSON.stringify(parsed));
  } catch (error) {
    console.log("Save read cutoff error:", error);
  }
};
...
const ChatScreen = ({ route, navigation }: Props) => {
      ...
      headerTitle: () => (
        <View style={styles.navHeaderContent}>
          {profileImage ? (
            ...
            >
              <Text style={[styles.navAvatarInitial, { color: colors.text }]}>
  ...
  return (
    ...
    >
        ...
        <FlatList
          ...
          ListFooterComponent={
            showTypingIndicator ? (
              ...
              >
                <Text
                  style={[styles.messageText, styles.theirMessageText, { color: colors.text }]}
                  numberOfLines={1}
                  ellipsizeMode="tail"
                >
                  {isAIChat &&
      ...
      {(!isAIChat && selectedChatAttachment) || (isAnalyzerMode && selectedAnalyzerFile) ? (
        <View style={[styles.attachmentStripRow, { backgroundColor: colors.card, borderTopColor: colors.border }]}>
          <View
            style={[
          ...
          >
            <View style={[styles.attachmentStripIconWrap, { backgroundColor: colors.chipBackground }]}>
              <FileText size={14} color={colors.primary} strokeWidth={2.3} />
            </View>
            <Text style={[styles.attachmentStripName, { color: colors.text }]} numberOfLines={1}>
            ...
            <TouchableOpacity
              onPress={() => {
                if (!isAIChat && selectedChatAttachment) {
                  setSelectedChatAttachment(null);
                  return;
                }
                if (isAnalyzerMode && selectedAnalyzerFile) {
                  setSelectedAnalyzerFile(null);
                }
              }}
              hitSlop={{ top: 8, right: 8, bottom: 8, left: 8 }}
            >
              <X size={16} color={colors.secondaryText} strokeWidth={2.3} />
            </TouchableOpacity>
          </View>
        </View>
      ) : null}
