...
const ExploreScreen = () => {
  ...
  const [otherStatusList, setOtherStatusList] = useState<StoryStatus[]>([]);
  const [otherStatusIndex, setOtherStatusIndex] = useState(0);
    ...
    try {
      ...
      const statusItems: StatusItem[] = await Promise.all(
        Array.from(groupedByUser.entries()).map(async ([uid, rows]) => {
          ...
          const orderedStatuses = [...rows].sort((a, b) => {
            const aTs = new Date(a.created_at || 0).getTime();
            const bTs = new Date(b.created_at || 0).getTime();
            return aTs - bTs;
          });
          ...
          const viewedByStatusId = new Map<number, boolean>();
          statusIds.forEach((statusId, index) => {
            viewedByStatusId.set(statusId, Boolean(viewedChecks[index]));
          });
          ...
          return {
            ...
            statuses: orderedStatuses.map((entry) => ({
              ...
              viewed: Boolean(viewedByStatusId.get(Number(entry.id)))
            }))
          };
        })
      );
  ...
  const markStatusViewed = async (statusId?: number) => {
    if (!statusId) return;
    try {
      const storedUserId = await AsyncStorage.getItem("userId");
      const viewerId = storedUserId ? Number(storedUserId) : null;
      if (!viewerId) return;
      await API.post("/mark-status-view", {
        status_id: statusId,
        viewer_id: viewerId
      });
    } catch (error) {
  ...
  const openOtherStatus = (item: StatusItem) => {
    const statuses = item.statuses?.length
      ? item.statuses
      : [
          {
            id: item.id,
    ...
    const firstUnseenIndex = statuses.findIndex((entry) => !entry.viewed);
    const startIndex = firstUnseenIndex >= 0 ? firstUnseenIndex : 0;

    setSelectedStatus(item);
    setOtherStatusList(statuses);
    setOtherStatusIndex(startIndex);
    setOtherStatusViewerVisible(true);
  };

  const goToNextOtherStatus = () => {
    setOtherStatusIndex((prev) => Math.min(prev + 1, Math.max(0, otherStatusList.length - 1)));
  };

  const goToPreviousOtherStatus = () => {
    setOtherStatusIndex((prev) => Math.max(prev - 1, 0));
  };

  const closeOtherStatusViewer = () => {
    setOtherStatusViewerVisible(false);
    setSelectedStatus(null);
    setOtherStatusList([]);
    setOtherStatusIndex(0);
    fetchStatuses();
  };

  useEffect(() => {
    if (!otherStatusViewerVisible) return;
    const activeStatus = otherStatusList[otherStatusIndex];
    if (!activeStatus?.id) return;
    markStatusViewed(activeStatus.id);
  }, [otherStatusViewerVisible, otherStatusList, otherStatusIndex]);

  const currentOtherStatus = otherStatusList[otherStatusIndex] || selectedStatus;
    ...
    >
      ...
      <View style={[styles.listBlock, { backgroundColor: colors.card }]}>
        ...
        >
          ...
          <TouchableOpacity
            style={[styles.myStatusAddBtn, { backgroundColor: colors.primary }]}
      ...
      <Modal
        ...
        animationType="fade"
        transparent
        onRequestClose={closeOtherStatusViewer}
      >
        <View style={styles.viewerBackdrop}>
          <View style={styles.viewerTopBar}>
            <Text style={styles.viewerTitle}>{selectedStatus?.name || "Status"}</Text>
            <TouchableOpacity onPress={closeOtherStatusViewer}>
              <Text style={styles.viewerClose}>Close</Text>
            </TouchableOpacity>
          </View>
          <View style={styles.viewerContent}>
            {currentOtherStatus?.mediaUrl ? (
              isVideoUrl(currentOtherStatus.mediaUrl) ? (
                <Video
                  key={`other-status-video-${currentOtherStatus.id}-${otherStatusIndex}`}
                  source={{ uri: currentOtherStatus.mediaUrl }}
                  style={styles.viewerVideo}
                  controls
                  repeat
                  resizeMode="contain"
                ...
                <Image source={{ uri: currentOtherStatus.mediaUrl }} style={styles.viewerImage} resizeMode="contain" />
              )
            ) : (
              <View style={styles.viewerEmpty}>
                <Text style={styles.viewerEmptyText}>No media found</Text>
            ...
            {otherStatusList.length > 1 ? (
              <>
                <TouchableOpacity
                  style={[styles.viewerNavBtn, styles.viewerNavLeft]}
                  onPress={goToPreviousOtherStatus}
                  disabled={otherStatusIndex === 0}
                >
                  <Text style={[styles.viewerNavText, otherStatusIndex === 0 && styles.viewerNavDisabled]}>{"<"}</Text>
                </TouchableOpacity>
                <TouchableOpacity
                  style={[styles.viewerNavBtn, styles.viewerNavRight]}
                  onPress={goToNextOtherStatus}
                  disabled={otherStatusIndex >= otherStatusList.length - 1}
                >
                  <Text
                    style={[
                      styles.viewerNavText,
                      otherStatusIndex >= otherStatusList.length - 1 && styles.viewerNavDisabled
                    ]}
                  >
                    {">"}
                  </Text>
            ...
            <View style={styles.viewerCaptionWrap}>
              {otherStatusList.length > 1 ? (
                <Text style={styles.viewerIndex}>
                  {otherStatusIndex + 1}/{otherStatusList.length}
                </Text>
              ) : null}
              <Text style={styles.viewerTime}>{currentOtherStatus?.time || ""}</Text>
              {!!currentOtherStatus?.caption && (
                <Text style={styles.viewerCaption}>{currentOtherStatus.caption}</Text>
              )}
            </View>
          </View>
        </View>
